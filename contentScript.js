(async () => {
    ...
    const enc_message_code =
        "-".repeat(20) + "discord-enc" + "-".repeat(20) + "\\n";
    const key_message_code =
        "-".repeat(20) + "discord-key" + "-".repeat(20) + "\n";
        ...
        bg_port.onMessage.addListener(async function (message) {
            if (
                ...
                bg_port.postMessage({
                    ...
                    name: output_message.channel_id,
                    message: message.real_msg,
                });
            }
            ...
            if (message.type === "create-key") {
                if (await ask_key_popup()) {
                    ...
                    api.sendMessage(
                        get_channel_id(),
                        exported_to_message(exported_key)
                    );
                }
            }
    ...
    function create_discord_button() {
        try {
            ...
            enc_button.onclick = async () => {
                ...
                const message_str = document
                    .querySelector('[class*="markup"][class*="editor"]')
    ...
    function create_button_on_message(
        message,
        text,
        func,
        bg_color = "#7CB342"
